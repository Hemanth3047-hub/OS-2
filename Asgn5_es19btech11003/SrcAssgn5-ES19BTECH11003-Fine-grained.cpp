...
ifstream f1;//file
ofstream f2;
sem_t block[10000];//semaphore for locking
  ...
  int k;
	...
	for(int i=TH[a][0];i<=TH[a][1];i++)//starting the loop
	{
		if(find_LM(a,elements[i])==0)//calling find_LM
			...
			for(int b=0;b<N;b++)//checking the edge
			   {
				if(Matrix[elements[i]][b]==1 || elements[i]==b)//locking all its adjacent vertices of external vertex
					sem_wait(&block[b]);//block.wait()
		...
		for(int j=0;j<N;j++)
		{
		  ...
		  if(find_LM(a,elements[i])==0)//calling find_LM
			...
			for(int b=0;b<N;b++)//checking the edge
			   {
				if(Matrix[elements[i]][b]==1 || elements[i]==b)	//locking all its adjacent vertices of external vertex
					sem_post(&block[b]);//block.signal()
				
				
			   }
			} 
		for(k=0;k<N;k++)//assigning minimum suitable colour
		{
			if(assign[k]!=true)
				break;
		}
		...
		colour[elements[i]]=k;//assigning the colour 
...
int main()
{
  ...
  f2.open("output.txt");
   for(int i=0;i<N;i++)
    sem_init(&block[i],0,1);//assigning to 1
...
f2<<"Time taken by the algorithm:"<<A2-A1<<"milli seconds"<<endl;//time
no_of_colours();//function
print();//function
